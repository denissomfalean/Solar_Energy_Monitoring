...
import {Button, ButtonGroup, Form, FormControl, FormSelect, InputGroup} from "react-bootstrap";

export const DeviceForm = (props) => {
    const [id, setId] = useState(props.data !== undefined ? props.data.id : undefined);
    ...
    return (
        <Form onSubmit={onSave}>
            ...
            <InputGroup className={"mt-2 mb-2"}>
                ...
                <FormControl type={"textarea"} onChange={e => setDescription(e.target.value)} value={description}
                             required/>
            </InputGroup>
            <InputGroup className={"mt-2 mb-2"}>
                <InputGroup.Text>Max energy</InputGroup.Text>
                <FormControl type={"input"} onChange={e => parseEnergyValue(e.target.value)} value={maxEnergy}
                             required/>
            </InputGroup>
            <Button type={"submit"} variant={"success"} className={"me-2"}>Save</Button>
            {id !== undefined ?
                <Button onClick={() => props.onDelete(id)} variant={"danger"} className={"me-2"}>Delete</Button> :
                <div/>}
        </Form>
    )
}